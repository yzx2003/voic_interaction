...
class VoiceNavigationSystem:
    def __init__(self):
        ...
        self.command_mapping = {
            ...
            # 按描述导航指令
            # "去拿一瓶水": CommandType.NAVIGATE,
            # "去取一本书": CommandType.NAVIGATE,
            # "到桌子旁边": CommandType.NAVIGATE,
            # "找水": CommandType.NAVIGATE,
            # "找书": CommandType.NAVIGATE,
    ...
    def find_point_by_environment(self, query):
        """根据查询文本匹配任务点的environment描述"""
        if not self.task_points:
            return None
        
        # 预处理查询文本：移除常见动词和无意义词汇
        stop_words = {'去', '拿', '取', '找', '到', '在', '往', '向', '的', '了', '是'}
        query_clean = re.sub(r'[^\u4e00-\u9fa5a-zA-Z0-9]', ' ', query)  # 保留文字和数字
        ...
        print(f"📌 提取的有效关键词: {query_words}")  # 调试日志
        ...
        for point_id, point_info in self.task_points.items():
            ...
            if not desc:
                continue  # 跳过没有环境描述的任务点
            
            # 检查查询关键词是否有任何一个完全包含在描述中
            matched_words = []
            for word in query_words:
                # 宽松匹配：关键词是描述的子字符串，或描述是关键词的子字符串
        ...
        # 降低匹配阈值，提高容错性
        if max_score < 0.3:  # 降低阈值，从0.5到0.3
    ...
    def send_goal_point(self, point_id):
        """发送目标点到robot_control话题使用goal_point_set（基于当前位姿的绝对位移）"""
        if point_id not in self.task_points:
            self.speak(f"未找到{point_id}号任务点")
            return False

        point_info = self.task_points[point_id]
        message = {
        ...
        try:
            ...
            # 循环模式下的增强语音提示
            if self.is_cycling:
                total_steps = len(self.cycle_order)
                current_step = self.current_cycle_step + 1  # 显示1-based步骤
                self.speak(
                    f"已前往{point_id}号点（循环步骤{current_step}/{total_steps}），"
                    f"等待机械臂完成工作"
                )
            else:
            ...
            #发送与point_id点相符的机械臂动作指令
            self.send_lerobot(int(point_id))
    ...
    # def send_goal_point(self, point_id):
    #     """发送目标点到robot_control话题使用move（基于当前位姿的相对位移）"""
    #     if point_id not in self.task_points:
    #         self.speak(f"未找到{point_id}号任务点")
    #         return False
    ...
    #     # 获取目标点信息
    #     point_info = self.task_points[point_id]
    #     target_pose = point_info["pose"]  # 目标点绝对位姿（x, y, yaw）
        
    #     # 获取当前位姿（通过MQTT实时更新的current_pose）
    #     current_x, current_y, current_yaw = self.current_pose

    #     print(f"获取到的当前位姿current_x: {current_x}current_y: {current_y}current_yaw: {current_yaw}，")

    #     # 计算相对位移（目标点 - 当前位姿）
    #     dx = target_pose["x"] - current_x  # x方向相对位移
    #     dy = target_pose["y"] - current_y  # y方向相对位移
    #     dz = target_pose["yaw"] - current_yaw  # yaw方向相对位移（可选，根据需求）
        
    #     # 构造move类型消息（vx, vy, vz为相对位移）
    #     message = {
    #         "cmd_type": "move",
    #         "vx": dx,
    #         "vy": dy,
    ...
    #     try:
    #         command_json = json.dumps(message)
    #         # 连续发布3次确保指令送达（MQTT QoS=2已保证可靠传输）
    #         for _ in range(3):
    #             self.mqtt_client.publish(
    #                 topic=self.mqtt_control_topic,
    #                 payload=command_json,
    #                 qos=2
    #             )
    #             time.sleep(0.05)  # 短间隔避免网络拥塞

    #         # 循环模式下的语音提示
    #         if self.is_cycling:
    #             total_steps = len(self.cycle_order)
    #             current_step = self.current_cycle_step + 1  # 1-based步骤
    #             self.speak(
    #                 f"已发送移动指令到{point_id}号点（循环步骤{current_step}/{total_steps}），等待机械臂完成工作"
    #             )
    #         else:
    #             self.speak(f"已发送移动指令到{point_id}号点（相对位移：x={dx:.2f}, y={dy:.2f}, yaw={dz:.2f}）")
                
    #         print(f"发送move指令到{point_id}: {message}")
    ...
    #         return True
    #     except Exception as e:
    #         print(f"发送move指令失败: {e}")
    #         self.speak("发送移动指令失败，请检查网络或参数")
    #         return False
        
    def start_cycle_navigation(self):
        """启动单次循环任务（1→2→3→0），完成后自动停止"""
        # 重置循环状态（确保多次启动循环时状态干净）
        self.is_cycling = True
        self.current_cycle_step = 0  # 初始步骤：对应1号点
        self.completed_cycles = 0    # 已完成循环次数归零
        self.speak("启动单次循环任务（1→2→3→0），完成后自动停止")
        
        # 向机械臂发送循环启动信号（数字6，连续3次）
        for i in range(3):
            self.send_lerobot(6)
            time.sleep(0.2)
        ...
        if first_point in self.task_points:
            self.send_goal_point(first_point)
        else:
            self.speak(f"循环启动失败：缺少{first_point}号任务点")
